...
当原版提示词的 `<output_format>` 包含多种格式类型时，**必须将格式选择逻辑放在"行为准则"中**，而不是"输出格式要求"中。
...
...
X) 输出选择：
    a) 分析类问题：使用'分析过程' + '结论'结构。
    b) 事实类问题：使用'回答' + '来源/验证'结构。
    c) 操作类问题：使用'步骤' + '注意事项'结构。
    d) 混合类问题：组合使用上述结构。
...
### 多场景格式处理
- [ ] 若原版有多种输出格式，格式选择逻辑放在"行为准则"中
- [ ] "输出格式要求"**不包含**多个并列的格式模板
- [ ] "输出格式要求"只包含通用格式规范（Markdown、表格等）
